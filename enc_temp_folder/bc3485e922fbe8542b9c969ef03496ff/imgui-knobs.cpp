    ...
    namespace detail {
        ...
        struct knob {
            ...
            void draw_needle(ImVec2 center, float radius, float angle, float length, float thickness, ImColor color) {
                ...
                auto angle_cos = cosf(angle + IMGUIKNOBS_PI * 0.5f);
                auto angle_sin = sinf(angle + IMGUIKNOBS_PI * 0.5f);
    ...
    bool BaseKnob(const char* label, ImGuiDataType data_type, DataType* p_value, DataType v_min, DataType v_max, float speed, const char* format, ImGuiKnobVariant variant, float size, ImGuiKnobFlags flags, int steps = 10) {
        ...
        switch (variant) {
            ...
            case ImGuiKnobVariant_Wiper: {
                ...
                // Add a needle
                float needleLength = 70.0f;  // Adjust the length of the needle as needed
                float needleThickness = 5.0f;  // Adjust the thickness of the needle as needed
                ImColor needleColor(1.0f, 0.0f, 0.0f); // You can change the color as needed

                knob.draw_needle(knob.center, 0.8f, knob.angle, needleLength, needleThickness, needleColor);
                break;
            }
           ...
           /*case ImGuiKnobVariant_Wiper: {
            ...
            /*case ImGuiKnobVariant_Needle: {
                ImVec2 needleCenter = ImVec2(knob.center.x, knob.center.y);
                float radius = 0.8f;
                float angle = knob.angle;
                float length = 50.0f;
                float thickness = 0.02f;

                // Convert the color_set to ImColor
                ImGuiKnobs::color_set primaryColorSet = detail::GetPrimaryColorSet();
                ImColor primaryColor(primaryColorSet.base);

                knob.draw_circle(0.85f, detail::GetSecondaryColorSet(), true, 32);
                knob.draw_needle(needleCenter, radius, angle, length, thickness, primaryColor);
                break;
            }*/
